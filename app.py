...
# 2. ฟังก์ชันโหลดข้อมูล (อ้างอิงชื่อไฟล์สั้นๆ ว่า data.csv)
@st.cache_data
def load_data():
    file_name = "data.csv"
...
try:
    ...
    if df is not None:
        ...
        st.error("❌ ไม่พบไฟล์ data.csv บน GitHub กรุณาเช็กชื่อไฟล์อีกครั้ง")
        st.info("คำแนะนำ: เปลี่ยนชื่อไฟล์บน GitHub ให้เป็น data.csv")
